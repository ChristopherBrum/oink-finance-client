...
const Transactions = () => {
  const [transactions, setTransactions] = useState<Transaction[]>(data);
	...
	const handleSearchTextFilter = (e: ChangeEvent<HTMLInputElement>) => {
		const currentValue = e.target.value;
		setSearchText(currentValue);
		filterTransactions({ text: currentValue });
	}
	...
	const handleDateFilter = (e) => {
		...
		setDate(currentDate);
		filterTransactions({ date: currentDate });
	}

	const filterTransactions = (filters: Filter) => {
		let temporaryTransactions = transactions;

		if (filters.text) {
			temporaryTransactions = filterTransactionsByDescriptionText(transactions, filters.text);
		}

		if (filters.date) {
			temporaryTransactions = filterTransactionsByDate(transactions, filters.date);
		}

		setFilteredTransactions(temporaryTransactions);
	}

	const filterTransactionsByDescriptionText = (transactions: Transaction[], currentValue: string) => {
		return transactions.filter(transaction => {
			const description = transaction.description.toLowerCase();
			const val = currentValue.trim().toLowerCase()
			return description.startsWith(val);
		});
	}

	const filterTransactionsByDate = (transactions: Transaction[], date: string) => {
		console.log(transactions[0].date, date);
		return transactions;
	}
        ...
        <form id={styles.searchForm}>
          <div className={styles.searchInputs}>
            <div className={styles.filterItems}>
              <label id={styles.textSearch} htmlFor="search">
                search by budget:
              </label>
              <input
                type="text"
                ...
                name="searchDescription"
								value={searchText}
								onChange={(e) => handleSearchTextFilter(e)}
              />
            </div>
            <div className={styles.filterItems}>
